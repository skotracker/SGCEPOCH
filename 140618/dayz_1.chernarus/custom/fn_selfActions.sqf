...
if (!isNull cursorTarget and !_inVehicle and !_isPZombie and (player distance cursorTarget < _allowedDistance) and _canDo) then {	//Has some kind of target
	...
	if((typeOf(cursortarget) == "VaultStorageLocked") and _ownerID == "0000" and (player distance _cursorTarget < 2)) then {
	   titleText ["Abandoned Safe! Code is 0000","PLAIN DOWN"]; titleFadeOut 2;
	};
	
	_ownerID = _cursorTarget getVariable ["CharacterID","0"];
	if((typeOf(cursortarget) == "LockboxStorageLocked") and _ownerID == "10099" and (player distance _cursorTarget < 2)) then {
	   titleText ["Abandoned Lockbox! Code is Red 99","PLAIN DOWN"]; titleFadeOut 2;
	};
	...
	*/
    ...
    if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
        if (s_player_clothes < 0) then {
            s_player_clothes = player addAction [("Take Clothes")+("</t>"), "custom\player_takeClothes.sqf",cursorTarget, -10, false, true, "",""];
        };
    } else {
...
//BIKE DEPLOY
if ("ItemToolbox" in _weapons && "PartGeneric" in _mags) then {
        hasBikeItem = true;
    } else { hasBikeItem = false;};
    if((speed player <= 1) && hasBikeItem && _canDo) then {
        if (s_player_deploybike < 0) then {
            s_player_deploybike = player addaction[("Deploy Bike") ,"custom\deploy.sqf","",5,false,true,"", ""];
        };
    } else {
...
if (s_player_deploybike2 < 0) then {
        s_player_deploybike2 = player addaction[("Pack Bike") ,"custom\pack.sqf","",5,false,true,"", ""];
    };
} else {
