...
_bloodAmount = 10000; // Amount of blood to give to player
_bloodbagUseTime = 19; // Amount of time it takes in second for the player to use the self bloodbag
...
_humanityAmount = 300; // Amount of humanity to give player if _humanityBool is true (250 is default for normal bloodbags)
...
if (dayz_combat == 1) then { // Check if in combat
	...
	while {r_doLoop} do {
		...
		if (r_interrupt) then {
			r_doLoop = true; // if interuppted turns loop off early so _finished is never true
		};
		sleep 0.1;
	};
	...
	if (_finished) then {
		...
		if (random(_infectionChance) < 1) then {
			...
			cutText [format["The bloodbag was infected! Blame Joe."], "PLAIN DOWN"]; //display text at bottom center of screen if infected
