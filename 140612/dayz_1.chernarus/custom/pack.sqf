...
// player is in combat and cant pack his bike
    cutText [format["You are in combat and cannot pack your bike"], "PLAIN DOWN"];
} else {
    if (typeOf cursorTarget == "MMT_Civ" || typeOf cursorTarget == "MMT_Civ") then {
    ...
    // delete it first to avoid player changing to another target
        ...
        cutText [format["You have to be facing your bike to pack it"], "PLAIN DOWN"];
    };
};